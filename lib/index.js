...
class Instagram{
    constructor(username,password){
        ...
        this.headers = {
            ...
            'x-ig-www-claim': 'hmac.AR3W0DThY2Mu5Fag4sW5u3RhaR3qhFD_5wvYbOJOD9qaPtoM',
    ...
    async getAfterLoginCookie(username){
        try{
            let userAgent = useragentFromSeed();
            const result = await this.requestNodeFetch.getCookie('https://www.instagram.com/'+username, this.headers);
            let ig_did = result.cookie.find((x)=> { return x.includes('ig_did')}) ? result.cookie.find((x)=> { return x.includes('ig_did')}).split(';')[0] : '';
            let csrf = result.cookie.find((x)=> { return x.includes('csrftoken')}) ? result.cookie.find((x)=> { return x.includes('csrftoken')}).split(';')[0] : '';
            let rur = result.cookie.find((x)=> { return x.includes('rur')}) ? result.cookie.find((x)=> { return x.includes('rur')}).split(';')[0] : '';
            let mid = result.cookie.find((x)=> { return x.includes('mid')}) ? result.cookie.find((x)=> { return x.includes('mid')}).split(';')[0] : '';
            let csrfToken = csrf.split('=')[1];
            let cookie = `${ig_did ? ig_did : `ig_did=${this.deviceId}`}; ${csrf}; ${rur}; ${mid}`;
            return {
                cookie: cookie,
                ig_did,
                csrf: csrfToken,
    ...
    async login(username, password){
        try{
            ...
            const body = `enc_password=${helpers.createEncPassword(password)}&username=${username}`;
    ...
    async registerSendOtp(phoneNumber){
        try {
            ...
            this.headers.referer = `${this.baseUrl}accounts/signup/phone`;
            this.headers["x-csrftoken"] = this.csrf.split('=')[1];
    ...
    async registerLastProcess(otpCode){
        try {
            ...
            this.headers.referer = `${this.baseUrl}accounts/emailsignup/`;
            this.headers["x-csrftoken"] = this.csrf.split('=')[1];
            const register = await this.requestNodeFetch.postResJsonWithCookie(`${this.baseUrl}accounts/web_create_ajax/`, dataString, this.headers);
            if (!register.cookie.find((x)=> { return x.includes('sessionid')})) {
                return register.response;
            }

            this.ds_user_id = register.cookie.find((x)=> { return x.includes('ds_user_id')}) ? register.cookie.find((x)=> { return x.includes('ds_user_id')}).split(';')[0] : '';
            this.sessionid = register.cookie.find((x)=> { return x.includes('sessionid')}) ? register.cookie.find((x)=> { return x.includes('sessionid')}).split(';')[0] : '';

            this.csrf = register.cookie.find((x)=> { return x.includes('csrftoken')}) ? register.cookie.find((x)=> { return x.includes('csrftoken')}).split(';')[0] : '';
            this.rur = register.cookie.find((x)=> { return x.includes('rur')}) ? register.cookie.find((x)=> { return x.includes('rur')}).split(';')[0] : '';
            this.ig_did = register.cookie.find((x)=> { return x.includes('ig_did')}) ? register.cookie.find((x)=> { return x.includes('ig_did')}).split(';')[0] : this.ig_did;
            this.mid = register.cookie.find((x)=> { return x.includes('mid')}) ? register.cookie.find((x)=> { return x.includes('mid')}).split(';')[0] : this.mid;
    ...
    async likeMediaByShortCodeAndType(shortCode, type = ''){
        try{
            let mediaId;
            switch (type) {
                case 'igtv':
                    this.headers.referer = `${this.baseUrl}tv/${shortCode}/`;
                    mediaId = await this._getMediaId(`${this.baseUrl}tv/${shortCode}/`);
                    break;
                case 'reel':
                    this.headers.referer = `${this.baseUrl}reel/${shortCode}/`;
                    mediaId = await this._getMediaId(`${this.baseUrl}reel/${shortCode}/`);
                    break;
                default:
                    this.headers.referer = `${this.baseUrl}p/${shortCode}/`;
                    mediaId = await this._getMediaId(`${this.baseUrl}p/${shortCode}/`);
                    break;
            }

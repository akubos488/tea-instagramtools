...
const getSuggestionUsername = (headers, username, firstName) => new Promise((resolve, reject) => {
    const dataString = `email=${username}&username=${username}&first_name=${firstName}&opt_into_one_tap=false`;
                            ...
                            if (otpCode1) {
                                ...
                                if (resultRegister.account_created) {
                                    ...
                                    try {
                                        ...
                                        // // do {
                                        // //     loginResult = await InstaClient.login(theRealUsername, password);
                                        // //     console.log(loginResult)
                                        // // } while ((loginResult.status && loginResult.status == 'fail') || !loginResult.authenticated);

                                        // // const 
                                        // console.log(loginResult)
                                        // if ((loginResult.status && loginResult.status == 'fail') || !loginResult.authenticated) {
                                        //     console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgRed,

...
const bikinnama = (gender = '1') => new Promise((resolve, reject) => {
    fetch(`https://fakenamecreator.com/?gender=${gender == '1' ? 'male' : 'female'}&locale=id_ID`, {
        method: 'GET'
    })
        .then(res => res.text())
        .then(result => {
            const $ = cheerio.load(result);
            const fullName = $('body > div > div > div.col-lg-9 > div:nth-child(4) > div.card-body > div > h3:nth-child(1) > b').text();
            const genderResult = gender == '1' ? 'Male' : 'Female'
...
(async () => {
    ...
    for (let index = 0; index < gender.length; index++) {
        console.log(`${index + 1}. ${gender[index]}`)
    }
    console.log('')
    const genderChoices = readlineSync.question('Pilih gender > ');


    console.log('')
    const proxyType = ['Residential Proxy ( Smartproxy )', 'Dedicated Datacenter ( Smartproxy )', 'Proxy List ( Standar )'];
    ...
    if (proxyTypeChoices == '2' || proxyTypeChoices == '3') {
        console.log(`Masukan nama file list proxy untuk ${proxyType[parseInt(proxyTypeChoices) - 1]}`);
        if (proxyTypeChoices == '3') {
            console.log(`Pastikan list proxy di dalam file beformat http://username:password@host:port`);
        }
        ...
        try {
            ...
            if (proxyTypeChoices == '2' || proxyTypeChoices == '3') {
                ...
                if (proxyTypeChoices == '3') {
                    ...
                    let proxyUrl = '';
                    if (dedicatedProxy.includes('http')) {
                        proxyUrl = dedicatedProxy;
                    } else {
                        proxyUrl = `http://${dedicatedProxy}`;
                    }
            ...
            if (balance > 1) {
                const indoName = await bikinnama(genderChoices);
                const name = indoName.fullName.toLowerCase().replace(/\s+/g, '')
                const username = name;
                console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen, `Menggunakan IP : ${ipResult.trim()}`, colors.Reset);
                console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen, `Gender di set ke : ${genderChoices == '1' ? 'Laki-Laki' : 'Perempuan'}`, colors.Reset);
                ...
                if (!username.includes("-")) {
                    ...
                    let suggestionName = await getSuggestionUsername(headers, username, indoName.fullName.split(' ')[0]);
                        ...
                        if (otpCode1) {
                            ...
                            do {
                                if (retryCount >= 3) {
                                    ...
                                    try {
                                        resultRegister = await InstaClient.registerLastProcess(otpCode);
                                        error = false;
                                    }catch(e){
                                        await delay(1000)
                                        error = true;
                            ...
                            if (resultRegister.account_created) {
                                ...
                                try {
                                    ...
                                    if (loginResult.response.hasOwnProperty('authenticated') && loginResult.response.authenticated == true) {
                                        fs.appendFileSync('./userpass.txt', `${theRealUsername}|${password}|${JSON.stringify(loginResult.headers)}\n`, 'utf8')
                                        console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen,
                                            `success login.`,
                                            colors.Reset);

                                            console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen,
                                            `detail login saved to => userpass.txt.`,
                                            colors.Reset);
                                        const payload = {
                                            biography: bioFinal,
                                        ...
                                        console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen,
                                            ...
                                            colors.Reset);
                                        try {
                                            const resultSetGender = await InstaClient.setGender(genderChoices);

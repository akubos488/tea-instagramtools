...
const irvankedeOTPApiKey = process.env.IRVANKEDEOTP_API_KEY;
...
const irvankedeotpGetServices = () => new Promise((resolve, reject) => {
    const formdata = new FormData();
    formdata.append("api_key", irvankedeOTPApiKey);

    fetch('https://irvankedeotp.com/api/services', {
        method: 'POST',
        body: formdata
    })
        .then(res => res.json())
...
const irvankedeotpGetStatus = (idPesanan) => new Promise((resolve, reject) => {
    const formdata = new FormData();
    formdata.append("api_key", irvankedeOTPApiKey);
    formdata.append("order_id", idPesanan);

    fetch('https://irvankedeotp.com/api/status', {
        method: 'POST',
        body: formdata
    })
        .then(res => res.json())
...
const irvankedeotpGetNumber = (serviceId) => new Promise((resolve, reject) => {
    const formdata = new FormData();
    formdata.append("api_key", irvankedeOTPApiKey);
    formdata.append("service_id", parseInt(serviceId.trim()));
    console.log(serviceId.trim())

    fetch('https://irvankedeotp.com/api/order', {
        method: 'POST',
        body: JSON.stringify({
            "api_key": irvankedeOTPApiKey,
...
const irvankedeotpUpdateStatus = (idPesanan, status) => new Promise((resolve, reject) => {
    const formdata = new FormData();
    formdata.append("api_key", irvankedeOTPApiKey);
    formdata.append("order_id", idPesanan);
    formdata.append("status", status);

    fetch('https://irvankedeotp.com/api/set_status', {
        method: 'POST',
        body: formdata
    })
        .then(res => res.json())
...
(async () => {
    try {
        if (!irvankedeOTPApiKey) {
            console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgRed, `pastikan sudah memasukan Api Key irvankedeotp pada file.env`, colors.Reset);
            process.exit(0);
        }

        const layananResult = await irvankedeotpGetServices();
        const servicesInstagramFilter = layananResult.data.filter(x => x.nama_aplikasi.toLowerCase().includes('instagram') && x.status == 'Aktif');
        const layananChoices = [];
        await servicesInstagramFilter.map((data, index) => {
            layananChoices.push(data.nama_aplikasi + `|id = ${data.id}|status = ${data.status}|otp tersedia = ${data.jumlah_tersedia}|harga idr = ${data.harga_idr}`);
        });

        console.log(colors.FgGreen, `
    
        Layanan Instagram yang tersedia dan Aktif di irvankedeotp.com
            ...
            [
                {
                    ...
                    choices: layananChoices
                }
            ])
            .then(async answers => {
                const answerSplit = answers.layanan.split('|');
                const servicesId = answerSplit[1].split('=')[1];
                console.log(`
Layanan Terpilih : 

Nama Layanan : ${answerSplit[0]}
Otp Tersedia : ${answerSplit[3].split('=')[1]}
Harga IDR : ${answerSplit[4].split('=')[1]}
                ...
                console.log("");
                const irvankedeotpGetNumberResult = await irvankedeotpGetNumber(servicesId);
                if (!irvankedeotpGetNumberResult.data.number) {
                    console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgRed, `Gagal mendapatkan nomer : ${irvankedeotpGetNumberResult.msg}`, colors.Reset);
                    process.exit(0);
                };

                const number = irvankedeotpGetNumberResult.data.number;
                const id = irvankedeotpGetNumberResult.data.id;
                await irvankedeotpUpdateStatus(id, 'processing')
                ...
                if (!username.includes("-")) {
                    ...
                    if (sendOtp.sms_sent) {
                        ...
                        let sessionOtp = 0;
                        do {
                            const irvankedeotpGetStatusResult = await irvankedeotpGetStatus(id);
                            otpCode1 = irvankedeotpGetStatusResult.data.otp;
                            ...
                            if (!otpCode1) {
                                ...
                                if (indexOtp >= 120000) {
                                    ...
                                    await irvankedeotpUpdateStatus(id, 'retry');
                                }
                            }

                            if (otpCode1) {
                                otpCode1 = irvankedeotpGetStatusResult.data.otp.split('+')[1] + irvankedeotpGetStatusResult.data.otp.split('+')[2];
                                await irvankedeotpUpdateStatus(id, 'done');
                            }
                        } while (!otpCode1);
                                    ...
                                    if (changeImageResult.changed_profile) {
                                        ...
                                        for (let index = 0; index < 2; index++) {
                                            ...
                                            if (resultAddPost.status && resultAddPost.status == 'ok') {
                                                console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgGreen,
                                                    `Sukses add post ke ${index + 1}.`,
                                                    colors.Reset);
                                            } else {
                                                console.log(`[ ${moment().format("HH:mm:ss")} ] `, colors.FgRed,
                                                    `gagal add post ke ${index + 1}, silahkan coba add post manual untuk user ${theRealUsername}`,
                                                    colors.Reset);
                                            }
                                        }
